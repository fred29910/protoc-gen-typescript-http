...
export function createSyntaxServiceClient(
  ...
  return {
    QueryOnly(request) { // eslint-disable-line @typescript-eslint/no-unused-vars
      ...
      if (
        request.repeatedString !== undefined && request.repeatedString !== null
      ) {
      ...
      if (
        request.nested?.string !== undefined && request.nested?.string !== null
      ) {
        queryParams.push(
          `nested.string=${
            encodeURIComponent(request.nested.string.toString())
          }`,
        );
      }
